...
### 1. No Positional Encoding (model.py, forward method)
**Severity: Critical** — The transformer has no positional encoding whatsoever. TabPFN-style models fundamentally depend on position to distinguish context (known) from query (unknown) tokens. Without positional information, the self-attention mechanism cannot learn which tokens are training examples vs. test examples. The `query_mask` tells the model *what* to predict, but the transformer has no way to learn positional patterns.

### 2. Label Leakage in Regression (model.py, line 69-70)
...
### 3. Double Softmax in Inference Print (inference.py, line 129)
...
### 4. Synthetic Prior Depth Variable Shadowing (model.py, line 105/109)
...
### 5. Unnormalized Fingerprints Break Feature Distribution (model.py + train.py)
...
### 6. No Attention Mask — Context Tokens Attend to Query Tokens (model.py)
...
### 7. Classification Cross-Entropy Shape Mismatch Risk (model.py, line 177)
...
### 8. Add Learnable Sinusoidal or Learned Positional Encoding
The ordering of molecules matters: context examples come first, queries come after. Adding positional encodings (sinusoidal or learned embeddings) would give the transformer the ability to learn "this is position i in the sequence" — essential for ICL.

### 9. Add Task-Specific Embedding per Dataset
...
### 10. Normalize Fingerprints for Better Gradient Flow
...
### 11. Use Proper Mask Token for Regression Queries
...
### 12. Add Causal/Structural Attention Mask
...
### 13. Increase max_classes for Classification Head
...
### 14. Weighted Loss for Classification Imbalance
...
1. ~~Add positional encoding to transformer input~~ — Added learned positional embedding (1024 positions)
2. ~~Use learnable mask token for regression (not 0.0)~~ — Regression now uses same `query_mask_token` as classification
3. ~~Add proper attention mask to transformer~~ — Query positions masked from key attention (no cross-query leakage)
4. ~~Fix double softmax in inference.py~~ — Removed erroneous `.softmax(dim=0)` on already-softmaxed output

### Phase 2: Performance Improvements ✅ COMPLETE
5. ~~Normalize fingerprints with log1p~~ — Added `torch.log1p(x_fp.clamp(min=0))` before fp_proj
6. ~~Increase max_classes to 4~~ — Updated default from 2 to 4
7. Add per-batch task embedding variation — deferred (no clear benefit)
8. ~~Fix depth variable shadowing~~ — Renamed loop variable to `n_layers`

### Phase 3: Optional Enhancements (deferred)
9. Add class-weighted loss — would need dataset-specific class distribution estimates
10. Consider molecule graph features beyond fingerprints — would require architectural changes
...
- Training step: ✅ (requires Lightning Trainer, tested in context)
